...
(function loadShell(){
  // Find base path from this script's src
  var basePath = '';
  var scripts = document.getElementsByTagName('script');
  for(var i=0;i<scripts.length;i++){
    var src = scripts[i].src||'';
    if(src.indexOf('config.js')!==-1){ basePath=src.replace('config.js',''); break; }
  }

  function inject(url, targetId, cb){
    fetch(url+'?v='+Date.now())
      .then(function(r){if(!r.ok)throw new Error(r.status);return r.text();})
      .then(function(html){
        var el=document.getElementById(targetId);
        if(el){el.innerHTML=html;if(typeof cb==='function')cb();}
      })
      .catch(function(err){console.warn('[Shell] '+url+':',err.message);});
  }
  ...
  document.addEventListener('DOMContentLoaded',function(){
    ...
    inject(basePath+'header.html','site-header',function(){hDone=true;check();});
    inject(basePath+'footer.html','site-footer',function(){fDone=true;check();});
  });
})();
