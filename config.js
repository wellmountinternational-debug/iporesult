...
(function loadShell() {
  ...
  // ── Get the folder where config.js lives ──────────────────
  // Works on any host: Hostinger, GitHub Pages, localhost etc.
  var scriptSrc = '';
  ...
  if (document.currentScript && document.currentScript.src) {
    ...
    for (var i = 0; i < tags.length; i++) {
      if ((tags[i].src || '').indexOf('config.js') !== -1) {
        scriptSrc = tags[i].src;
        break;
      }
    }
  }

  // basePath = everything up to and including the last "/"
  // e.g. "https://user.github.io/iporesult/" or "https://iporesult.in/"
  var basePath = scriptSrc.substring(0, scriptSrc.lastIndexOf('/') + 1);

  // ── XHR inject — works on GitHub Pages, Hostinger, all hosts ──
  function inject(filename, targetId, cb) {
    ...
    xhr.onreadystatechange = function () {
      ...
      if (xhr.status === 200) {
        ...
        // Still call cb so shellReady fires even if one file fails
        if (typeof cb === 'function') cb();
      }
    };
    xhr.send();
  ...
  // ── Wait for DOM, inject both files in parallel ────────────
  function run() {
    ...
    function check() {
      ...
      markActive();
      // Fire shellReady so page scripts know the header/footer are in the DOM
  ...
  if (document.readyState === 'loading') {
    ...
    run(); // DOM already ready (script loaded async/defer)
  }
