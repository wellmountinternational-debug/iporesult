...
// ★ FAST single-IPO detail — one GAS call instead of 4 parallel calls
// Returns { open, upcoming, gmp, subs } for the IPO detail page
function getIPODetail(name) {
  ...
  return fetch(url)
    .then(function(r){ if(!r.ok) throw new Error('HTTP '+r.status); return r.json(); })
    .then(function(data){ _cacheSet(cacheKey, data); return data; })
    .catch(function(err){ console.warn('[GAS] getIPO failed:', err.message); return {open:null,upcoming:null,gmp:null,subs:[]}; });
}
