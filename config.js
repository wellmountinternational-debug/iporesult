...
function gasGet(action, params) {
  ...
  return fetch(url)
    ...
    .catch(function(err){ console.warn('[GAS]', action, err.message); return []; });
}
...
function getIPODetail(name) {
  ...
  return fetch(url)
    .then(function(r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
    .then(function(data) {
      // If GAS returned an error (old deployment doesn't know getIPO)
      // OR returned empty, fall back to 4 parallel calls
      if (data && data.error) throw new Error(data.error);

      // Valid response — check if it has the right shape
...
(function applySavedTheme() {
  var saved = localStorage.getItem('ipo_theme') || 'dark';
  document.documentElement.setAttribute('data-theme', saved);
})();

// ── LAST UPDATED DISPLAY ─────────────────────────────────────
function setLastUpdated(dateStr) {
  var el = document.getElementById('navLastUpdated');
  if (!el) return;
  var d = dateStr ? new Date(dateStr) : new Date();
  var options = { day:'2-digit', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit', timeZone:'Asia/Kolkata' };
  el.textContent = '🕐 ' + d.toLocaleString('en-IN', options) + ' IST';
  el.title = 'Data last updated: ' + d.toLocaleString('en-IN', options) + ' IST';
}
...
function alertFor(name){
  // Use Firebase alert modal if firebase.js is loaded
  if (typeof showAlertModal === 'function') { showAlertModal(name); return; }
  // Fallback
